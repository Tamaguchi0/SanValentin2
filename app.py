...
def EncontrarMiembro(s):
    patrones = [
        r'([\w\s\(\)~★♡🎷🐛«»]+):',  # Nombre con caracteres especiale
        '([\w]+):',                                # Nombre
...
RutaChat = 'Data/WhatsApp Chat with tu.txt'
...
# nota, hacer que esto sea una variable modificable desde la pagina 
...
# aqui poner mensajes relevantes o importantes 




st.header('Estadísticas generales 🤓☝️')
# poner algo aqui para que el usuario sdseleccione de que fdecha a que fehca 
...
stopwords = STOPWORDS.update(['que', 'qué', 'con', 'de', 'te', 'en', 'la', 'lo', 'le', 'el', 'las', 'los', 'les', 'por', 'es',
                              'son', 'se', 'para', 'un', 'una', 'su', 'si','nos', 'ya', 'hay', 'esta',
                              'pero', 'del', 'eso', 'este', 'como', 'así', 'todo', 'https','Media','omitted',
                              'y', 'mi', 'o', 'q', 'yo', 'al', 'estoy', 'voy', 'porque', 'ni', 'sé', 'null' ])
...
st.write('Basado en el trabajo de [Sandreke](https://linktr.ee/sandreke99) en este [tutorial](https://youtu.be/2eDccEfyQeM) 🎥')
